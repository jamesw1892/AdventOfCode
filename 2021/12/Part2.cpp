...
bool setContains(std::set<std::string> s, std::string key) {
 ...
 */
int explore(std::shared_ptr<Cave> start, std::map<std::string, std::shared_ptr<Cave>> caves, std::string pathSoFar, std::set<std::string> &existingPaths) {

    // successful path
    if (start->Name == "end") {
        if (!setContains(existingPaths, pathSoFar)) {
            printf("Path: %s\n", pathSoFar.c_str());
            existingPaths.insert(pathSoFar);
            return 1;
        } else {
            return 0;
        }
    ...
    for (std::shared_ptr<Cave> neighbour: start->Neighbours) {
        ...
        if (neighbour->Big || !neighbour->Visited) {
            ...
            numPaths += explore(neighbour, caves, pathSoFar + "," + neighbour->Name, existingPaths);
            neighbour->Visited = false;

        // if small and already visited and can be visited twice
        } else if (neighbour->CanBeVisitedTwice) {
            neighbour->CanBeVisitedTwice = false;
            numPaths += explore(neighbour, caves, pathSoFar + "," + neighbour->Name, existingPaths);
            neighbour->CanBeVisitedTwice = true;
        }
    }
    return numPaths;
}
...
int numPaths(std::string filename) {
    ...
    std::vector<std::shared_ptr<Cave>> smallCaves;
    ...
    for (std::string line: readLines(filename)) {
        ...
        if (mapContains(caves, from)) {
            ...
            if (islower(from.at(0)) && from != "start") {
                smallCaves.push_back(caveFrom);
            }
        }
        ...
        if (mapContains(caves, to)) {
            ...
            if (islower(to.at(0)) && to != "start") {
                smallCaves.push_back(caveTo);
            }
        }
    ...
    // repeat for each small cave being allowed to be visited twice
    std::set<std::string> existingPaths;
    int numPaths = 0;
    for (std::shared_ptr<Cave> smallCave: smallCaves) {
        smallCave->CanBeVisitedTwice = true;
        numPaths += explore(caveStart, caves, "start", existingPaths);
        smallCave->CanBeVisitedTwice = false;
    }
    return numPaths;
}

int main() {
    // assertEqual(numPaths("input_test.txt"), 36);
    // assertEqual(numPaths("input_test2.txt"), 103);
    // assertEqual(numPaths("input_test3.txt"), 3509);
    printf("Ans = %d\n",  numPaths("input.txt"));
    return 0;
}