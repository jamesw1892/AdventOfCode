 ...
 */
bool mapContains(std::map<char, unsigned long long> m, char key) {
    return m.find(key) != m.end();
}

unsigned long long p2(std::string filename) {
    ...
    // perform steps
    for (int stepNum = 1; stepNum <= 40; stepNum++) {
        std::string current = templat;
        for (unsigned long long pos = 1; pos < templat.length(); pos++) {
    ...
    // default to 0 so can use unsigned. The true value can never be 0 since
    // then it wouldn't be the min/max value (it doesn't appear)
    unsigned long long min = 0;
    unsigned long long max = 0;
    for (std::map<char, unsigned long long>::iterator it = counts.begin(); it != counts.end(); it++) {
        if (min == 0 || it->second < min) {
            min = it->second;
        }
        if (max == 0 || it->second > max) {
            max = it->second;
        }
    }
...
int main() {
    ...
    assertEqual(p2("input_test.txt"), 2188189693529);
    printf("Ans = %llu\n", p2("input.txt"));
