...
fn run_part1(filename: &str) {
    ...
    for line in file_lines {
        let nums: Vec<usize> = line.split("   ").map(|s: &str| s.parse().expect(format!("Failed to parse '{}' to usize", s).as_str())).collect();
        left_list.push(nums[0]);
        right_list.push(nums[1]);
    }
    left_list.sort();
    right_list.sort();
    let total_distance: usize = left_list.into_iter().zip(right_list).map(|(n1, n2)| n1.abs_diff(n2)).sum();
...
fn run_part2(filename: &str) {
    ...
    let similarity_score: usize = left_list.into_iter().map(|n1: usize| n1 * right_list.contains(&n1)).sum();
